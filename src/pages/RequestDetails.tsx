...
const RequestDetails: React.FC = () => {
  ...
  const handleCompanyOwnerSubmit = () => {
    ...
    const successMessage = `Successfully processed ${decisions.length} item${decisions.length > 1 ? 's' : ''}: ${approvedItems.length} approved, ${rejectedItems.length} rejected, ${revertedItems.length} reverted`;
    ...
    console.log('Company Owner Decisions:', { requestId: requestData.id, decisions });
  };
  ...
  const handleSupervisorSubmit = () => {
    ...
    let successMessage = `Successfully updated ${updates.length} item${updates.length > 1 ? 's' : ''}`;
    if (orderedCount > 0) successMessage += ` - ${orderedCount} ordered`;
    if (partialCount > 0) successMessage += ` - ${partialCount} partially received`;
    if (receivedCount > 0) successMessage += ` - ${receivedCount} fully received`;
    ...
    // In real app, save to backend here
    console.log('Supervisor Status Updates:', { requestId: requestData.id, updates });
  };
  ...
  const handleStatusChange = (newStatus: string, additionalData?: any) => {
    ...
    const updatedData = {
      ...requestData,
      status: newStatus,
      selectedVendors: newStatus === 'approved' ? selectedVendors : requestData.selectedVendors
    };

    if (additionalData) {
      // Add receipt history or other additional data
      if (newStatus === 'partially_received' || newStatus === 'material_received') {
  ...
  const handleResubmit = () => {
    ...
    toast({
      ...
      description: "Request has been resubmitted for approval with updated vendor quotations",
    });

    // In real app, save to backend here
    console.log('Request Resubmitted:', { requestId: requestData.id, updatedData });
  };

  const handleVendorQuotationChange = (itemId: string, quotations: VendorQuotation[]) => {
    if (!requestData) return;
  ...
  const shouldShowStatusDropdown = () => {
    ...
    if (currentUser.role === 'site_supervisor') {
      return ['approved', 'ordered', 'partially_received'].includes(requestData.status);
    }
    ...
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              {currentUser?.role === 'company_owner' ? 'Review and approve request' : 'Manage request status'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {shouldShowStatusDropdown() && currentUser?.role !== 'company_owner' && (
            <StatusDropdown
              currentStatus={requestData.status}
              userRole={currentUser?.role as 'company_owner' | 'site_supervisor'}
              onStatusChange={handleStatusChange}
              requestId={requestData.id}
              hasVendorSelected={Object.keys(selectedVendors).length === requestData.items.length}
            />
          )}

          {currentUser?.role === 'site_supervisor' && requestData.status === 'reverted' && (
      ...
      <div className="w-full">
        ...
        <div className="space-y-6">
          {currentUser?.role === 'company_owner' && requestData.status === 'pending_approval' ? (
            ...
            <RequisitionIndentForm
              ...
              machines={machines}
            />
          ) : currentUser?.role === 'site_supervisor' && ['approved', 'ordered', 'partially_received', 'issued'].includes(requestData.status) ? (
