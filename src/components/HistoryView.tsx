                ...
                {historyData.map((item) => (
                  <TableRow key={item.id} className='hover:bg-gray-50'>
                    ...
                    <TableCell className='font-medium'>
                      {item.purchaseValue} - ({item.previousMaterialValue} per{' '}
                      {item.perMeasureQuantity})
                    </TableCell>

                    <TableCell className='font-medium'>
                      {item.requestedValue} ({item.currentValue})
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
    ...
    <Card className='border border-gray-200 w-full'>
      <CardHeader>
        <CardTitle className='flex items-center gap-2 text-lg'>
          ...
          {title ||
            `Last 5 Material Received - ${materialName || 'Material'} (${
              location || 'Location'
            })`}
                    ...
                    <div className='space-y-3'>
                      ...
                      <div>
                        ...
                        <div className='font-medium'>
                          {item.purchaseValue} - ({item.previousMaterialValue}{' '}
                          per {item.perMeasureQuantity})
                        </div>
                      </div>
                    </div>
