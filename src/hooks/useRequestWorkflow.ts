...
export const useRequestWorkflow = () => {
  ...
  const [requests, setRequests] = useState<Map<string, RequestWorkflowData>>(new Map());

  // Initialize request workflow data
  const initializeRequest = useCallback((requestId: string, initialData: Partial<RequestWorkflowData>) => {
  ...
  const updateRequestStatus = useCallback((
    ...
    setRequests(prev => {
      ...
      const statusHistory: RequestStatusHistory = {
        ...
        user: userContext?.name || updateData.approvedBy || updateData.revertedBy || updateData.receivedBy || 'System',
        data: updateData
      };
      ...
      const updated: RequestWorkflowData = {
        ...current,
        status: newStatus,
        statusDescription: updateData.statusDescription || current.statusDescription,
      ...
      if (newStatus === 'material_received' || newStatus === 'partially_received') {
  ...
  const approveRequest = useCallback((requestId: string, approverName: string) => {
  ...
  const revertRequest = useCallback((requestId: string, revertReason: string, reverterName: string) => {
  ...
  const resubmitRequest = useCallback((requestId: string, submitterName: string) => {
  ...
  const getRequestWorkflow = useCallback((requestId: string): RequestWorkflowData | undefined => {
  ...
  const canPerformAction = useCallback((
    ...
    switch (action) {
      case 'approve':
      case 'revert':
        return userRole === 'company_owner' && request.status === 'pending_approval';
